...
## Key Features
- **Save/Load Config**: Saves and loads the full form state (machine settings, IO sections, all job specifiers) as an XML file; Save Config / Load Config buttons appear below "Add IO Section" in GuiMachineSpecifier; serialization via EdeConfigManager (javax.xml); all GuiJobSpecifier fields have matching setters for loading

## Recent Changes
- 2026-04-25: Removed `Pointer<Value>` from interpreter symbol tables: `Environment.java` and `InterpreterEnvironment.java` now use `SymbolTable<Value>` directly; `lookupVariable()` returns `Value`; `addVariable()` stores `Value` without wrapping; all `Ptr.deRefrence()` calls replaced with direct variable use; all `Ptr.assign(val)` / `Utils.shallowAssignIdent(ptr, val)` calls replaced with `val.setValue(exp)` throughout `Interpreter.java`, `VerilogInterpreter.java`, and `EdeInterpreter.java`; `import ede.stl.common.Pointer` removed from all four files. Fixed `setValue` implementations: `VectorVal.setValue` now calls `Utils.shallowAssign(this, exp.longValue())`; `Node.setValue` sets `this.stateSignal = stat.boolValue()` (inherited by RegVal and all gate subclasses); `EdeStatVal.setValue` calls `this.setStatusValue(exp.intValue())`; `EdeRegVal.setValue` calls `this.gui.setRegisterValue(this.regString, val.longValue())`.

- 2026-03-29: Fixed IncompatibleClassChangeError in generated Verilog bytecode: all 6 `INVOKEVIRTUAL` calls on `"ede/stl/values/Value"` (an interface) changed to `INVOKEINTERFACE` with `isInterface=true` — affected `boolValue()` in if/if-else/for/while/ternary and `intValue()` in repeat-loop and VectorVal field init; this caused runtime crashes in any generated process method using conditionals or loops
- 2026-03-25: Save/Load Config feature: EdeConfigManager.java (XML serialization with javax.xml); Save Config + Load Config buttons added below "Add IO Section" in GuiMachineSpecifier; setters added to GuiJobSpecifier (setJobType, setJobName, setSyntaxHighlighting, setImportsText, setVerilogPath, setVerilogInputFile, setVerilogMainModule, setExePath, setKeywordFilePath, setJarPaths); setInputText added to GuiEdeGenField; loadFromConfig added to GuiJobSpecifierList; addIoSectionRowWithValues helper for programmatic IO section creation; fixed CompiledEnvironment.java RunableThread→RunnableThread typo in EdeStl submodule
