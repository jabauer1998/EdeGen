...
## Recent Changes
- 2026-04-26: Pushed the `Value`-construction logic for register reads down into `GuiRegister.GetRegisterValue()` itself (now returns `ede.stl.values.Value` instead of `long`). It parses its own `JLabel` text per `regFormat` to a raw `long`, builds a `VectorVal(width-1, 0)` sized by its own `RegisterDecimalLength` (clamped to `[1..64]`), fills each bit via `vec.setValue(i, new RegVal(((raw >> i) & 1L) != 0))`, then returns `Utils.getOptimalUnsignedForm(vec.longValue())`. `GuiRegisterFile.getRegisterValue(...)` is now a thin delegator that just calls `GuiRegister.GetRegisterValue()` directly (the previous `toOptimalValue` helper and `RegVal`/`VectorVal`/`Utils` imports were removed from `GuiRegisterFile`).
